...
def mail_user(email_id):
    ...
    msg['To'] = email_id
    msg['Subject'] = subject
    body='Hi there, sending this email from Python to send the images from Google .'

    msg.attach(MIMEText(body,'plain'))

    #filename='resized_dir_zip.zip'
    attachment  =open('image_gray_and_resized.zip','rb')

    part = MIMEBase('application','octet-stream')
    part.set_payload((attachment).read())
    encoders.encode_base64(part)
    part.add_header('Content-Disposition',"attachment; filename= "+'image_gray_and_resized.zip')
